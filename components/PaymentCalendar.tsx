...
function fmtCompact(v: number | undefined): string {
  ...
  const intPart = new Intl.NumberFormat("ru-RU", {
    maximumFractionDigits: 0,
  }).format(Math.trunc(v));
  const decPart = Math.abs(Math.round((v % 1) * 100))
    .toString()
    .padStart(2, "0");
  ...
  return (
    <div className="shrink-0 mx-4 lg:mx-6 mb-2">
      ...
      >
        ...
        <span
          className={`text-sm font-bold tabular-nums ml-1 ${total < 0 ? "text-rose-600" : "text-slate-800"}`}
        >
          {fmtFull(total)}
        </span>
        {manualBalance && (
        ...
        <span className="ml-auto text-slate-400 shrink-0">
          {accountsOpen ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
        </span>
      </button>
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50">
                <th className="text-left px-4 py-1.5 text-[10px] font-medium text-slate-400 uppercase tracking-wide">
                  Счёт
                </th>
                <th className="text-right px-3 py-1.5 text-[10px] font-medium text-slate-400 uppercase tracking-wide">
                  ПланФакт
                </th>
                <th
                  className="text-right px-3 py-1.5 text-[10px] font-medium text-slate-400 uppercase tracking-wide"
                  style={{ width: "140px" }}
                >
                  Банк
                </th>
              ...
              {visible.map((acc) => {
                ...
                return (
                  <tr key={acc.id} className="hover:bg-slate-50/50">
                    <td
                      className="px-4 py-2 text-slate-700 truncate"
                      style={{ maxWidth: "200px" }}
                    >
                      {acc.name}
                    </td>
                    <td
                      className={`px-3 py-2 text-right font-semibold tabular-nums whitespace-nowrap ${acc.balance < 0 ? "text-rose-600" : acc.balance === 0 ? "text-slate-400" : "text-slate-800"}`}
                    >
                      {fmtFull(acc.balance)}
                    </td>
                    <td className="px-3 py-1.5" style={{ width: "140px" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setPerAccountValue(acc.id, rawInput(e.target.value))
                        }
          ...
          {accounts.length !== visible.length && (
            <div className="border-t border-slate-100 px-4 py-1.5">
              ...
              >
                {showZero
                  ? "↑ Скрыть нулевые"
                  : `↓ Показать ещё ${accounts.length - nonZero.length} нулевых`}
              </button>
            </div>
          )}
            ...
            <table className="w-full text-[12px]">
              <tbody>
                <tr>
                  ...
                  <td
                    className={`px-3 py-2.5 text-right font-bold tabular-nums whitespace-nowrap ${total < 0 ? "text-rose-600" : "text-slate-800"}`}
                  >
                    {fmtFull(total)}
                  </td>
                  <td className="px-3 py-1.5" style={{ width: "140px" }}>
                    <input
                      ...
                      onChange={(e) =>
                        setManualBalance(rawInput(e.target.value))
                      }
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="w-full text-left px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white hover:border-slate-300 truncate"
      >
        {selected ? (
          selected.name
        ) : (
...
function NewPaymentRequestModal({
  day,
  month,
  year,
  userId,
      ...
      >
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200">
          <h3 className="font-bold text-slate-800">
            Запрос на выплату — {pad2(day)}.{pad2(month)}.{year}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-100 rounded-lg border border-slate-200"
            >
              Отмена
            </button>
            <button
              ...
              className="flex-1 px-3 py-2.5 text-sm text-white bg-teal-600 hover:bg-teal-700 rounded-lg font-medium disabled:opacity-50"
            >
...
export const PaymentCalendar: React.FC = () => {
  ...
  const [perAccountBalances, setPerAccountBalances] = useState<
  ...
  const [incomePlanManual, setIncomePlanManual] = useState<
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `/api/payment-calendar?month=${monthStr}&dateMode=${dateMode}`,
        {
  ...
  const saveCalendarBalance = useCallback(
    async (accountId: number | null, value: string, isTotal: boolean) => {
      try {
  ...
  const saveIncomePlanDay = useCallback(
    async (day: number, amount: number) => {
      try {
  ...
  useEffect(() => {
    load();
    loadIncomePlan();
    loadCalendarBalances();
  }, [load, loadIncomePlan, loadCalendarBalances]);
  ...
  const handleCopyFromPrevMonth = async () => {
    ...
    try {
      ...
      showToast(
        result.copied > 0
          ? `Скопировано запросов: ${result.copied}`
          : "Оплаченных запросов в прошлом месяце не найдено",
      );
      ...
      return {
        ...
        expenseCategories: prev.expenseCategories.map((cat) => {
          ...
          const newDays = {
            ...cat.days,
            [srcDay]: remaining,
            [targetDay]: [...targetEntries, ...moving],
          };
  ...
  async function exportXlsx() {
    ...
    try {
      const r2 = await fetch("/api/initial-data", {
        headers: { "x-user-id": String(user?.id || "") },
      });
      if (r2.ok) {
    ...
    if (allCats.length === 0)
      allCats = data.expenseCategories.map((c) => ({ id: c.id, name: c.name }));
    ...
    addDataRow(
      "  План",
      [nv(totalIncomePlan), ...days.map((d) => nv(incomePlanManual[d] || 0))],
      { fg: "FFFFFF", fgTotal: "F0FDF4", textColor: "475569" },
    );
    addDataRow(
      "  Ожидаемый факт",
      [
        nv(totalIncomeUnconfirmed),
        ...days.map((d) => nv(data.incomePlan[d] || 0)),
      ],
      {
        fg: "F8FAFC",
        fgTotal: "D1FAE5",
    ...
    addDataRow(
      "  Факт",
      [nv(totalIncomeFact), ...days.map((d) => nv(data.incomeFact[d] || 0))],
      {
        fg: "F8FAFC",
        fgTotal: "DCFCE7",
        textColor: "166534",
    ...
    addDataRow(
      "  План",
      [nv(totalExpensePlan), ...days.map((d) => nv(data.expensePlan[d] || 0))],
      { fg: "FFFFFF", fgTotal: "FFF5F5", textColor: "475569" },
    );
    addDataRow(
      "  Факт",
      [nv(totalExpenseFact), ...days.map((d) => nv(data.expenseFact[d] || 0))],
      {
        fg: "F8FAFC",
        fgTotal: "FFE4E6",
        textColor: "9F1239",
    ...
    addDataRow(
      "  План",
      [nv(totalBalancePlan), ...days.map((d) => nv(balancePlan[d] || 0))],
      { fg: "FFFFFF", fgTotal: "EFF6FF", textColor: "3B82F6" },
    );
    addDataRow(
      "  Прогноз",
      [
        nv(totalBalanceUnconfirmed),
        ...days.map((d) => nv(data.balanceUnconfirmed?.[d] || 0)),
      ],
      {
        fg: "F8FAFC",
        fgTotal: "E0F2FE",
    ...
    addDataRow(
      "  Факт",
      [nv(totalBalance), ...days.map((d) => nv(data.balance[d] || 0))],
      {
        fg: "EFF6FF",
        fgTotal: "DBEAFE",
        textColor: "1D4ED8",
    ...
    for (const cat of allCats) {
      ...
      const catRow = ws.addRow([
        ...
        ...days.map((d) => {
          const es = cd?.days[d];
          return es ? nv(cellTotal(es)) : null;
        }),
      ]);
        ...
        for (const e of entries) {
          ...
          row.eachCell({ includeEmpty: true }, (cell, col) => {
            ...
            cell.fill = {
              type: "pattern",
              pattern: "solid",
              fgColor: { argb: isStatus ? statusFg : rowBgColor },
            };
            cell.alignment = {
              horizontal: col === 1 ? "center" : col === 3 ? "right" : "left",
              vertical: "middle",
  ...
  const totalIncomeUnconfirmed = days.reduce(
    (s, d) => s + (data?.incomePlan[d] || 0),
    0,
  );
  const totalIncomePlan = days.reduce(
    (s, d) => s + (incomePlanManual[d] || 0),
    0,
  );
  const totalIncomeFact = days.reduce(
    (s, d) => s + (data?.incomeFact[d] || 0),
    0,
  );
  const totalIncomeAll = days.reduce(
    (s, d) => s + (data?.incomeAll?.[d] || 0),
    0,
  );
  const totalExpensePlan = days.reduce(
    (s, d) => s + (data?.expensePlan[d] || 0),
    0,
  );
  const totalExpenseFact = days.reduce(
    (s, d) => s + (data?.expenseFact[d] || 0),
    0,
  );
  const totalExpenseAll = days.reduce(
    (s, d) => s + (data?.expenseAll?.[d] || 0),
    0,
  );
  const totalBalance =
    (data?.startingBalance ?? 0) + totalIncomeFact - totalExpenseFact;
  const totalBalanceUnconfirmed =
    (data?.startingBalanceUnconfirmed ?? data?.startingBalance ?? 0) +
    totalIncomeAll -
    totalExpenseAll;
  ...
  // For past days (already happened) Plan balance uses actual confirmed income + actual paid
  // expenses so it aligns with Fact. For future days it uses the manual income plan + PR plan.
  const isPastMonth =
  ...
  const balancePlan: Record<number, number> = (() => {
    ...
    for (const d of days) {
      const isPastOrToday = isPastMonth || (isCurrentMonth && d <= todayDay);
      // Income: always use the manually entered plan (user edits this row directly)
      const incomeForDay = incomePlanManual[d] || 0;
      // Expense: for past/today use actual paid transactions (by t.date, same as Fact)
      // so that PRs paid on a different day than planned don't distort the balance.
      // For future days use planned PR amounts.
      const expenseForDay = isPastOrToday
        ? data?.expenseFact[d] || 0
        : data?.expensePlan[d] || 0;
      running += incomeForDay - expenseForDay;
      result[d] = running;
    }
    return result;
  })();

  const totalBalancePlan =
    days.length > 0
      ? (balancePlan[days[days.length - 1]] ?? planStartingBalance)
      : planStartingBalance;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] overflow-hidden">
      <div className="shrink-0 px-4 lg:px-6 pt-3 pb-2 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-0.5 bg-white border border-slate-200 rounded-lg px-1 py-0.5 shadow-sm">
            <button
              onClick={prevMonth}
              className="p-1 hover:text-teal-600 text-slate-400 hover:bg-slate-50 rounded transition-colors"
            >
              <ChevronLeft size={14} />
            </button>
      ...
      {error && (
        <div className="mx-4 lg:mx-6 bg-rose-50 border border-rose-200 rounded-xl p-3 text-rose-700 text-sm shrink-0">
          {error}
        </div>
      )}
      ...
      {data && (
        <AccountBalancesPanel
          ...
          setPerAccountValue={(id, v) =>
            setPerAccountBalances((prev) => ({ ...prev, [id]: v }))
          }
          onSaveAccountBalance={(accountId, value) =>
            saveCalendarBalance(accountId, value, false)
          }
      ...
      {data && !loading && (
        ...
        >
          <table
            ...
            style={{ minWidth: LABEL_W + TOTAL_W + days.length * COL_W }}
          >
            <thead>
              <tr>
                <th
                ...
                {days.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="opacity-70" style={{ fontSize: 9 }}>
                        {DAY_SHORT[dow]}
                      </div>
                      {dragState && dragOverDay === d && (
            ...
            <tbody>
              ...
              <EditableSummaryRow
                label="План"
                tooltip="Ручной прогноз доходов. Нажмите на любой день и введите ожидаемую сумму. Используется для строки «Остаток: План» внизу."
              ...
              <SummaryRow
                ...
                tooltip="Все зафиксированные доходы: и уже подтверждённые, и ещё ожидающие подтверждения. Показывает, сколько денег должно поступить в этот день."
              ...
              <SummaryRow
                label="Факт"
                tooltip="Только подтверждённые доходы. Эти деньги точно поступили на счёт (доходы со статусом «Проверено»)."
              ...
              <SummaryRow
                label="План"
                tooltip="Все запланированные расходы: заявки на выплату и прямые расходы (комиссии, РКО и тд.). Показывает, сколько планируется потратить в этот день."
              ...
              <SummaryRow
                label="Факт"
                tooltip="Расходы, которые уже оплачены и проверены (статус «Оплачено» / «Проверено»). Деньги, которые реально ушли со счета."
              ...
              <BalanceRow
                label="План"
                tooltip="Ручной план доходов, плюс начальные остатки месяца, минус все запланированные расходы."
              ...
              <BalanceRow
                label="Прогноз"
                tooltip="Ожидаемые доходы(Ожидаемый факт) минус все запланированные расходы. Показывает, что будет на счету, если оплатить операции по пла."
              ...
              <BalanceRow
                label="Факт"
                tooltip="Реальный остаток: только подтверждённые доходы и фактически оплаченные расходы."
                total={totalBalance}
                days={days}
                values={data.balance}
                todayDay={todayDay}
              ...
              {catsOpen &&
                data.expenseCategories.map((cat, catIdx) => {
                  const catTotal = Object.values(cat.days)
                    .flat()
                    .reduce((s, e) => s + e.amount, 0);
                  return (
                    <tr
                    ...
                    >
                      ...
                      <td
                        ...
                        style={{
                          ...
                          width: TOTAL_W,
                          minWidth: TOTAL_W,
                          background: catIdx % 2 === 0 ? "#f8fafc" : "#f1f5f9",
                        }}
                      >
                        ...
                        return (
                          ...
                          >
                            ...
                            `}
                              ...
                              draggable
                              onDragStart={(e) =>
                                handleDragStart(e, cat.id, d, entries)
                              }
                              onDragEnd={handleDragEnd}
                              onMouseEnter={(e) =>
                                !dragState &&
                                showTooltip(entries, cat.name, d, e)
                              }
                              onMouseLeave={hideTooltip}
                              onClick={() =>
                                handleChipClick(entries, cat.name, cat.id, d)
                              }
                            >
                              <div
                                className={`font-bold text-center leading-tight ${cfg.text}`}
                                style={{ fontSize: 10 }}
                              >
                                {fmtCompact(total)}
                              </div>
                              <div className="flex justify-center items-center gap-0.5 mt-0.5">
                                {statuses.map((s) => (
                                  <span
                                    key={s}
                                    className={`w-1 h-1 rounded-full ${(STATUS_CFG[s] || STATUS_FALLBACK).dot}`}
                                  />
                                ))}
                                {entries.length > 1 && (
                                  <span
                                    className={`text-[8px] font-bold leading-none opacity-70 ${cfg.text}`}
                                  >
                                    ×{entries.length}
                                  </span>
                                )}
        ...
        <div className="flex-1 mx-4 lg:mx-6 mb-4 rounded-xl border border-slate-200 bg-white overflow-hidden animate-pulse">
          ...
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-9 border-b border-slate-100 flex">
              <div
                style={{ width: LABEL_W + TOTAL_W }}
                className="bg-slate-50 border-r border-slate-100"
              />
        ...
        <div className="fixed inset-0 z-[99990] flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 w-80 flex flex-col gap-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Скопировать все{" "}
              <span className="font-semibold text-emerald-700">оплаченные</span>{" "}
              запросы из <span className="font-semibold">{prevMonthName}</span>{" "}
              в <span className="font-semibold">{MONTH_NAMES[month - 1]}</span>?
              <br />
              <br />
              Даты будут сдвинуты на&nbsp;+1&nbsp;месяц. Новые запросы будут
              созданы со статусом «Ожидает».
            </p>
      ...
      {toasts.length > 0 && (
        <div className="fixed bottom-5 right-5 flex flex-col gap-2 z-[99999] pointer-events-none">
          {toasts.map((t) => (
            ...
            >
              <span
                className={`w-2 h-2 rounded-full flex-shrink-0 ${t.type === "success" ? "bg-emerald-500" : "bg-rose-500"}`}
              />
              {t.message}
            </div>
          ))}
      ...
      {tooltip &&
        (() => {
          ...
          const tipLeft = Math.max(
            8,
            Math.min(tipCenter - TIP_W / 2, window.innerWidth - TIP_W - 8),
          );
          return (
            <div
              ...
              style={{
                ...
                width: TIP_W,
                maxHeight: showBelow
                  ? Math.min(TIP_MAX_H, spaceBelow)
                  : Math.min(TIP_MAX_H, spaceAbove),
              }}
            >
              <div className="px-3 py-2 border-b border-slate-100 bg-slate-50 rounded-t-xl flex-shrink-0">
                ...
                <div className="text-[10px] text-slate-400">
                  {tooltip.day} {MONTH_NAMES_GEN[month - 1]} {year}
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <span
                          className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${cfg.pill}`}
                        >
                          <span
                ...
                {tooltip.entries.length > 5 && (
                  <div className="pt-2 border-t border-slate-100 text-center">
                    <span className="text-[10px] text-slate-400 italic">
                      и ещё {tooltip.entries.length - 5} операций…
                    </span>
                  </div>
                )}
                {tooltip.entries.length > 1 && (
                  <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-slate-700">
                      {fmtFull(cellTotal(tooltip.entries))}
                    </span>
                  </div>
                )}
              </div>
...
const GroupHeader: React.FC<GroupHeaderProps> = ({
  ...
  <tr>
    ...
    >
      <span style={{ color: GROUP_ACCENT[accent].color, marginRight: 4 }}>
        {icon}
      </span>
      {label}
    </td>
  ...
  <tr className="border-b border-slate-100" style={{ background: rowBg }}>
    ...
    >
      <span className="inline-flex items-center gap-0.5">
        {label}
        {tooltip && <RowTooltip text={tooltip} />}
      </span>
    </td>
    <td
    ...
    >
      {total ? (
        <FmtWithCents
          v={total}
          className={`${textClass} ${bold ? "font-bold" : "font-medium"}`}
        />
      ) : (
    ...
    {days.map((d) => {
      ...
      return (
        <td
          ...
          style={{
            background: isToday ? todayBg : rowBg,
            opacity: past && !v ? 0.4 : 1,
          }}
        >
          {v > 0 && (
            <FmtWithCents
            ...
            {isEdit ? (
              ...
              >
                {v > 0 ? (
                  ...
                  <span
                    className="text-slate-200 group-hover:text-emerald-300 transition-colors"
                    style={{ fontSize: 10 }}
                  >
                    +
                  </span>
...
const BalanceRow: React.FC<BalanceRowProps> = ({
  ...
  <tr className="border-b border-slate-200" style={{ background: rowBg }}>
    ...
    >
      <FmtWithCents
        v={total}
        className={total >= 0 ? "text-slate-700" : "text-slate-700"}
      />
    </td>
    {days.map((d) => {
      const v = values[d] ?? 0;
